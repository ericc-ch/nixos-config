...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, signal) {
      const result = await exaMcpCall("web_search_exa", {
        query: params.query,
        type: "auto",
  ...
  pi.registerTool({
    name: "codesearch",
    label: "Code Search",
    description: "Search for code examples, API documentation, and programming guides using Exa AI. Use for any programming-related question.",
    parameters: Type.Object({
      query: Type.String({ description: "Search query (e.g., 'React useState examples')" }),
      tokensNum: Type.Optional(Type.Number({ description: "Token count 1000-50000 (default: 5000)" })),
    }),
    async execute(_id, params, signal) {
      const result = await exaMcpCall("get_code_context_exa", {
        query: params.query,
