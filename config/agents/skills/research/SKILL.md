...
Do the research **yourself in this session** — do not spin up a background subagent.
...
5. **Write It Down:** Capture the synthesized findings in a single Markdown file, citing each claim's source. Save it where the repo already keeps such notes; match the existing convention, and if there is none, put it somewhere sensible and tell the user where.
6. **Comprehensive Delivery:** Keep the synthesized findings in your context and deliver a highly detailed, well-sourced final response to the user only once the research is complete.
