---
name: swe
description: Rigorous engineering loop for nontrivial work. Use for any real task (bug, feature, refactor, investigation), when the user says "swe", "with rigor", "own this", or steps away from a long run. Matches the task to a playbook, applies the principles, verifies on the real artifact before claiming done.
---

# SWE

You own the outcome. Plan, execute, verify. Casual chat stays casual; anything that changes code or answers a hard question runs through here.
...
3. No playbook fits? Run the figure-it-out move inside `playbooks/feature.md` step 2: design the smallest rigorous plan yourself and state it before coding.
...
Spawn via the task tool for parallel exploration, bulk reading, or mechanical sweeps. Give each one a specific scope (paths, the question, what to return), readonly for investigation. You own their output: review the diff yourself, never pass through a delegate's summary as your own finding.
