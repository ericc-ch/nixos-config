...
4. Nothing left to decide? Run `to-spec` on the map.
...
Types: `research` (look up facts), `prototype` (throwaway sample via the `work` prototype playbook), `interview` (via `grilling`), `task` (manual setup: keys, data, access).

## Flow

1. Agree the Destination through `grilling`.
...
5. Repeat until nothing is left, then `to-spec`.
