...
**Read the prior trail; do not redo it.** For "take over this", "continue where X left off", resuming from a session note, or landing on a pushed branch meant to be continued.
...
1. Locate the prior trail: `wiki/works/*/session.md`, specs, tickets, and maps, or the pushed branch itself. If destination is already in the tree and `works/` is gone, start from `wiki/CONTEXT.md` and git; missing `session.md` is expected. Read the summary first, then scan back for decision points. Long transcripts get parsed in a readonly subagent; keep the reduced timeline in the main thread.
