...
- Set up the Effect language service:
  ...
  - Point your editor at the workspace TypeScript version so tsserver loads the plugin. In VS Code, run TypeScript: Select TypeScript Version → Use Workspace Version. In Neovim, use vtsls or another tsserver-plugin-aware LSP.
  - Read the [language-service README](https://github.com/Effect-TS/language-service) for editor-specific setup details.

## Errors and tags

- Define domain errors with `Data.TaggedError("NotFound")<{ ...fields }>` or `Schema.TaggedError<NotFound>()("NotFound", { ...fields })`. The tag is the first constructor argument. The runtime sets `_tag` from it. Never write `_tag` by hand.
